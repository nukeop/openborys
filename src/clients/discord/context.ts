...
import { createAssistantMessage, createUserMessage } from '../../services/ai-tools';
...
export const mapDiscordMessageToModelMessages = (message: DiscordMessage): ModelMessage[] => {
  ...
  const messagePart: TextPart = { type: 'text', text: `[${timestamp}]${authorPrefix}: ${message.cleanContent}` };
  ...
  if (!isOwnMessage) {
    modelMessages.push(createUserMessage({ content: [messagePart, ...imageParts] }));
  }

  return modelMessages;
}

export const getLastMessages = async (message: DiscordMessage,
  contextWindowSize: number): Promise<ModelMessage[]> => {
  const rawMessages = await message.channel.messages.fetch({
    limit: contextWindowSize,
  });
  const messages = rawMessages.reverse().map(mapDiscordMessageToModelMessages).flat();
